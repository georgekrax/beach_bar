...
//       if (customer) {
//         try {
//           await customer.customSoftRemove(true);
//         } catch (err) {
//           return res.status(400).send({ error: err.message }).end();
//         }
//       }
...
//   try {
//     event = stripe.webhooks.constructEvent(req.body, sig, process.env.STRIPE_PAYMENT_WEBHOOKS_SECRET);
//   } catch (err) {
//     return res.status(400).send(`Webhook Error: ${err.message}`);
//   }
...
//               "cart",
//               "cart.products",
//               "cart.products.startTime",
//               "cart.products.endTime",
