  ...
  definition(t) {
    t.nullable.field("getStripeConnectOAuthUrl", {
      ...
      resolve: async (_, __, { payload, res, stripe }: MyContext): Promise<string | null> => {
        ...
        const user = await User.findOne({ where: { id: payload.sub }, relations: ["owner", "account", "account.country"] });
        if (!user || !user.owner || !user.account || !user.account.country || !user.account.city) return null;
