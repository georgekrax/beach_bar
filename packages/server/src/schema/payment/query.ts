...
export const PaymentQuery = extendType({
  type: "Query",
  definition(t) {
    t.nullable.list.field("getPayments", {
      ...
      resolve: async (_, { monthId, year }, { payload }: MyContext): Promise<TPaymentVisits[] | null> => {
        monthId = parseInt(monthId);
        if (!payload || !payload.sub) return null;
        if (monthId < 1 || monthId > 12)
          throw new UserInputError("Invalid monthId. Please provide a value between 1 and 12", { code: errors.INVALID_ARGUMENTS });
    ...
    t.field("payment", {
      ...
      args: {
        refCode: stringArg({ description: "The referral code of the payment to fetch" }),
      },
    ...
    t.float("paymentRefundAmount", {
      description: "Get the amount of refuned of a specific payment / trip",
      args: {
        refCode: stringArg({ description: "The referral code of the payment to fetch" }),
      },
