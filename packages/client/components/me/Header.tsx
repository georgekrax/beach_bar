...
const SearchBoxDynamic = dynamic(() => {
  const prom = import("@/components/Search/Box").then(mod => mod.Box);
  return prom;
});
...
export const Header: React.FC<Props> = ({ withAuth = true, searchBar = false, sticky = true, ...props }) => {
  ...
  return (
    ...
    <header className={styles.container + " w100"} {...props}>
      <div className="container--padding container--mw h100 flex-row-space-between-center">
        ...
        {searchBar && <SearchBoxDynamic className={styles.searchBar} inHeader />}
        {/* {withAuth && data?.me ? ( */}
        {withAuth ? (
          <div className="flex-row-center-center">
            {/* <div className={styles.userFirstName}>Hello {data?.me?.firstName}!</div> */}
            <div className={styles.userFirstName}>Hello {"George"}!</div>
            {/* <Avatar /> */}
          </div>
        ) : (
          <div>Login</div>
